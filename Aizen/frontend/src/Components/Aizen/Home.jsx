...
const HomePage = () => {
  ...
  const images = [
    `${API_BASE}/assets/images/banner-1.png`,
    `${API_BASE}/assets/images/banner-2.png`,
    `${API_BASE}/assets/images/banner-3.png`,
  ];
    ...
    const fetchData = async () => {
      try {
        const [categoriesResponse, productsResponse] = await Promise.all([
          axios.get(`${import.meta.env.VITE_BASE_URL}/controller/Admin/getCategories.php`),
          axios.get(`${import.meta.env.VITE_BASE_URL}/controller/productDetail.php?store_id=${storeId}`),
        ]);
        ...
        const all = productsResponse.data?.allProducts || [];
        const popular = productsResponse.data?.popularProducts || [];

        setProducts(shuffleArray(all));
        // Fallback: if no popular products flagged, show top of all products
        setPopularProducts(popular.length ? popular : all.slice(0, 10));
        setVisibleProducts(all.slice(0, 10));
      } catch (error) {
...
>
  {popularProducts.slice(0, 10).map((product) => (
    <div key={product.id} className="p-2" onClick={() => handleProductClick(product.id)}>
        ...
        <section className="p-4 mb-4" style={{ boxShadow: 'none', backgroundColor: 'transparent' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
            {categories.map((category) => (
              ...
              >
                <img
                  src={`${API_BASE}/assets/images/${category.image}`}
                  alt={category.name}
      ...
      {/* Meet Our Team section temporarily disabled */}
      {false && (
      <section className="w-full m-0 shadow-none bg-primary-green/10 ">
        <h2 className="text-4xl font-extrabold text-gray-900 text-center mb-16 tracking-tight">Meet Our Team</h2>
