...
const ProductsList = ({ searchTerm, storeId }) => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        // Use centralized API client which applies baseURL from env
        ...
        const message = err.response
          ? `Error fetching products: ${err.response.status} ${err.response.statusText}`
          : `Error fetching products: ${err.message || 'Network error'}`;
        setError(message);
