...
const Header = ({ onSearch, onStoreChange }) => {
  ...
  useEffect(() => {
    ...
    const fetchStores = async () => {
      try {
        const response = await client.get('/Admin/getStores.php');
    ...
    const fetchCartItemCount = async (userId) => {
      ...
      if (token && userId) {
        try {
          const response = await client.get('/getCartItemsCount.php', {
            headers: {
    ...
    const fetchAvatar = async (uid) => {
      ...
      if (!token || !uid) return;
      try {
        const res = await client.get('/profile/read_profile.php', {
    ...
    const checkAuth = async () => {
      ...
      if (token) {
        ...
        try {
          ...
          fetchAvatar(response.data.user.id);
    ...
    // Also listen to storage changes (in case of multi-tab)
    const onStorage = (ev) => {
      if (ev.key === 'avatarUrl' && ev.newValue) {
