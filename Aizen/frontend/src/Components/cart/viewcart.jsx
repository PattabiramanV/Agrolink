...
const ViewCart = () => {
  ...
  const fetchCartItems = async (userId) => {
    try {
      const response = await client.get('/cart.php', {
        params: { id: userId },
      });
      ...
      setError(err.response ? `Error fetching cart items: ${err.response.status} ${err.response.statusText}` : 'Error fetching cart items');
    }
  };
  ...
  const deleteCartItem = async (cartItemId) => {
    ...
    try {
      const response = await client.delete('/cart.php', {
                    ...
                    <tr key={product.cart_item_id} className="relative group hover:bg-gray-50 transition-colors duration-200">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div className="flex items-center">
                          <img
                            src={`${API_BASE}/assets/images/${product.images}`}
                            alt={product.product_name}
                            className="w-16 h-16 object-cover mr-4 rounded"
                            onError={(e) => {
                              e.target.onerror = null; // Prevents infinite loop if error occurs
                              e.target.src = `${API_BASE}/assets/images/placeholder.jpg`; // Placeholder image
                            }}
                          />
