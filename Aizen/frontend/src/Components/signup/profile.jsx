...
const Profile = ({ setProfileOpen }) => {
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      ...
      try {
        const response = await client.get('/Admin/getUserDetails.php', {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const response = await client.get('/profile/read_profile.php', {
          headers: { Authorization: `Bearer ${token}` },
        ...
        if (avatar) {
          // Normalize to absolute URL if backend returns relative path
          let absolute = /^https?:\/\//i.test(avatar) || avatar.startsWith('data:')
            ? avatar
            : `${API_BASE}/${avatar.replace(/^\//, '')}`;
          // Map legacy localhost:8000 URLs to current API_BASE
          absolute = absolute.replace('http://localhost:8000/controller', `${API_BASE}/controller`);
    ...
    try {
      ...
      if (avatarFile) {
        ...
        const uploadResponse = await client.post(
          ...
          {
            headers: {
              // Let axios set multipart/form-data with proper boundary
              Authorization: `Bearer ${token}`
            }
          }
        );
        ...
        if (uploadedUrl) {
          // Normalize uploaded URL to current API base if it contains localhost:8000
          const normalized = uploadedUrl.replace('http://localhost:8000/controller', `${API_BASE}/controller`);
          setImageUrl(normalized);
          try { localStorage.setItem('avatarUrl', uploadedUrl); } catch {}
          // Notify header to update immediately
          window.dispatchEvent(new CustomEvent('avatar-updated', { detail: { url: normalized } }));
        }
        else {
      ...
      const msg = error.response?.data?.message || 'Failed to update profile or upload avatar';
      toast.error(msg);
