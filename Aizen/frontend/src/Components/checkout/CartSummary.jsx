...
const CartSummary = ({ onTotalChange, onUserIdChange }) => {
  ...
  const fetchCartItems = async (userId) => {
    ...
    try {
      ...
      setError(error.response ? `Error fetching cart items: ${error.response.status} ${error.response.statusText}` : 'Error fetching cart items');
      setItems([]);
    }
  };
            ...
            {items.map(item => (
              <li key={item.cart_item_id} className="flex py-6 border-b border-gray-200">
                <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                  <img
                    src={`${API_BASE}/assets/images/${item.images}`}
                    alt={item.name}
